...
- Contact: David Richter (`David.Richter.26@nd.edu`)
  ...
  Link to dataset: owner-supplied CRC submission script and verification namelist; validated 2026-08-31 on CRC, 128 MPI ranks, ~1026s total, with load imbalance (25%) and MPI communication (22%) as the largest cost categories.
...
- Contact: Jon MacArt (`jmacart@nd.edu`)
  ...
  Link to dataset: none needed for the 2D shear layer verification case (grid built in code); smoke test validated 2026-09-14 (CPU-only, 1 vs 8 threads).
...
- Contact: Mark Caprio (`mcaprio@nd.edu`)
  ...
  Link to dataset: repository's included `example-transitions-01` case, validated 2026-08-31 on CRC (4 MPI ranks x 8 OpenMP threads), output matched the checked-in reference. Mark offered larger "heavy duty" production input; not yet requested.
...
- Contact: Tarrick Qahash (`tqahash@nd.edu`)
  ...
  Link to dataset: owner-supplied CSV; minimal run and 1000-permutation run both validated 2026-09-07, chromosome 7 peak confirmed by Tarrick as expected for chloroquine. Scope still to choose: modernize to `rqtl2`, or parallelize VCF/gene-interval processing (internal decision, not blocked on Tarrick).
...
- Contact: Bharat Mishra (`bmishra2@nd.edu`)
  ...
  Link to dataset: built-in `test` profile only so far (smoke test, ~25 MB); validated end to end on Grid Engine and HTCondor 2026-09-14. Representative dataset, reference genome, and a specific performance question still need to come from Bharat.
...
- Contact: Nirjhar Bhattacharyya (`nbhattac@nd.edu`)
  ...
  Link to dataset: owner-supplied SNP table already in hand; baseline validated 2026-09-14 with a clear bottleneck (MAD outlier filter, 86% of runtime).
  Status: have the data, waiting for GitHub repo access; just followed up, expected to close quickly.
...
- Contact: Michael Quintieri (`mquintie@nd.edu`) and Laura Fields (`lfields2@nd.edu`)
  ...
  Status: Michael is seeking approval from his collaboration to share code and data (as of 2026-09-09).
...
- Contact: Peter Kogge (`Peter.M.Kogge.1@nd.edu`)
  ...
  Link to code: not yet received
  Link to dataset: not yet received
  Status: no reply since 2026-08-25 despite two follow-ups (2026-09-04, forwarded to Doug 2026-09-09).
