...
casename = 'vortex_vgwavy_2D_periodic'
#casename = 'vortex_vgwavy_2D_nonperiodic'
...
for i in range(0,6):
    ...
    # Compute error
    if i>0:
        nx,ny = np.shape(q[:,:,0]); print(nx,ny)
...
plt.loglog(nx_list,1e2*np.array(nx_list,dtype=np.float64)**(-4),'k--',
           label='$O(h^4)$')
plt.xlabel('$N$')
